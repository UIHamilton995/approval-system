...
//       setFilteredProviders(data.result.slice(0, 10)); // Initial slice
//     } catch (error) {
...
//   // Filter providers with proper debouncing
//   useEffect(() => {
...
//           provider.ProviderCode?.toLowerCase().includes(searchTerm)
//         );
//       }).slice(0, 10); // Filter first, then slice
...
//       }
//     };
//   }, [providerSearchTerm, allProviders]); // Proper dependencies
...
const ExclusionForm = ({ addTask }) => {
  ...
  const fetchAllProviders = useCallback(async () => {
    // Check cache first
    const cachedProviders = localStorage.getItem('cachedProviders');
    if (cachedProviders) {
      setAllProviders(JSON.parse(cachedProviders));
      setFilteredProviders(JSON.parse(cachedProviders).slice(0, 10));
      return;
    }
    ...
    try {
      ...
      localStorage.setItem('cachedProviders', JSON.stringify(providers));
    } catch (error) {
      console.error("Error fetching providers:", error);
      setToastMessage("Error fetching provider list. Please try again.");
      setShowSuccessToast(true);
  ...
  // Optimized provider filtering with relevance scoring
  useEffect(() => {
    if (debounceTimeoutRef.current) {
      clearTimeout(debounceTimeoutRef.current);
    }
    ...
    debounceTimeoutRef.current = setTimeout(() => {
      ...
      // Score providers based on match relevance
      const scoredProviders = allProviders.map(provider => {
        ...
        // if (name === searchTerm) score += 1;
        // if (code === searchTerm) score += 1;
        
        // Bonus for starts with
        if (name.startsWith(searchTerm)) score += 1;
        if (code.startsWith(searchTerm)) score += 1;
  ...
  useEffect(() => {
    ...
    const fetchAllCompanies = async () => {
      try {
        ...
        const data = await response.json();
        setAllCompanies(data.result || []);
        setFilteredCompanies(data.result.slice(0, 10));
      } catch (error) {
        console.error("Error fetching companies:", error);
        setToastMessage("Error fetching company list. Please try again.");
        setShowSuccessToast(true);
  ...
  const fetchEnrollees = useCallback(async (groupId) => {
    ...
    try {
      ...
      const data = await response.json();
      const enrollees = data.result_overview || [];
      setAllEnrollees(enrollees);
      setFilteredEnrollees(enrollees.slice(0, 10));
    } catch (error) {
      console.error("Error fetching enrollees:", error);
      setToastMessage("Error fetching enrollee list. Please ensure the company is valid and try again.");
      setShowSuccessToast(true);
                ...
                filteredProviders.map((provider) => (
                  ...
                  >
                    <div className="font-medium">
                      {highlightMatch(provider.FullName || 'Unknown Provider', providerSearchTerm)}
                    </div>
                    <div className="text-sm text-gray-600 flex justify-between flex-wrap">
                      <span>Code: {highlightMatch(provider.ProviderCode || 'N/A', providerSearchTerm)}</span>
                      {provider.BankName && <span>Bank: {provider.BankName}</span>}
                    </div>
                  </div>
                ))
            ...
            <div className="absolute z-20 w-full mt-1 bg-white border rounded-md shadow-lg max-h-60 overflow-y-auto">
              {filteredCompanies.length > 0 ? (
                filteredCompanies.map((company) => (
                  ...
                  >
                    <div className="font-medium">
                      {highlightMatch(company.GROUP_NAME || "Unknown Company", companySearchTerm)}
                    </div>
                  </div>
                ))
              ) : (
              ...
              <div className="absolute z-20 w-full mt-1 bg-white border rounded-md shadow-lg max-h-60 overflow-y-auto">
                {filteredEnrollees.length > 0 ? (
                  filteredEnrollees.map((enrollee) => (
                    ...
                    >
                      <div className="font-medium">
                        {highlightMatch(enrollee.FullName || "Unknown Enrollee", enrolleeSearchTerm)}
                      </div>
                      {enrollee.EnrolleeID && (
                        <div className="text-sm text-gray-600">
                          ID: {highlightMatch(enrollee.EnrolleeID, enrolleeSearchTerm)}
                        </div>
                      )}
                    </div>
                  ))
